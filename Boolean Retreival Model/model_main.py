...
import pygame
import sys
from Inverted_Index import process_boolean_query
from Positional_Index import process_proximity_query, positional_index
...
pygame.display.set_caption("Boolean Retreival Model")
...
def main_event():
    ...
    while running:
        ...
        if state == "start_screen":
            ...
            draw_text("INFORMATION RETRIEVAL ASSIGNMENT 1", font, (211, 211, 211), screen, SCREEN_WIDTH // 2, 50)
            ...
            if '/' in query:
                result, term1, term2, distance, number = process_proximity_query(query, positional_index)
            else:
                result, number = process_boolean_query(query)
                result = [int(i.split('.')[0]) for i in result]

            draw_text("Resulting Documents and terms: " + str(number), font, (211, 211, 211), screen, 400, (SCREEN_HEIGHT / 2) + 40)

            # Adjusting height and width of results box docids
            list_width = 300
            list_height = (SCREEN_HEIGHT / 2) + 80
            for i in sorted(result):
                draw_text(str(i), font, (211, 211, 211), screen, list_width, list_height)
                list_height += 20
